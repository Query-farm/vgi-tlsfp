...
#
# Run this repo's sqllogictest suite (test/sql/*.test) against the vgi-tlsfp
...
# is what LOCATION (VGI_TLSFP_WORKER) the .test files ATTACH:
#
#   subprocess  VGI_TLSFP_WORKER = the stdio worker command (DuckDB spawns it).
#   http        start `tlsfp-worker --http` (auto port; advertises `PORT:<n>`
#               on stdout), VGI_TLSFP_WORKER = http://127.0.0.1:<port>.
#               If VGI_TLSFP_WORKER is ALREADY set to an http(s):// URL (e.g. a
#               pre-launched container in the docker image_test), it is used
#               as-is and no local worker is spawned.
#   unix        start `tlsfp-worker --unix <sock>` (advertises `UNIX:<sock>`
#               on stdout), VGI_TLSFP_WORKER = unix://<sock>.
#
# Required environment:
#   HAYBARN_UNITTEST  path to the haybarn-unittest binary
#   WORKER_BIN        path to the compiled tlsfp-worker binary (used to launch
#                     the http/unix servers, and the stdio LOCATION). Falls back
#                     to VGI_TLSFP_WORKER when that is a bare command (subprocess).
# Optional:
...
WORKER_BIN="${WORKER_BIN:-$REPO/target/release/tlsfp-worker}"
...
# Bring up the out-of-band server for http/unix and resolve VGI_TLSFP_WORKER.
...
start_server_and_set_location() {
  local kind="$1"
  : "${WORKER_BIN:?path to the tlsfp-worker binary (WORKER_BIN)}"
  ...
  case "$kind" in
    http)
      ...
      export VGI_TLSFP_WORKER="http://127.0.0.1:$port"
      echo "HTTP worker ready on 127.0.0.1:$port (pid $SERVER_PID)"
      ;;
    unix)
      SOCK_PATH="${VGI_TLSFP_SOCK:-/tmp/tlsfp.$$.sock}"
      ...
      export VGI_TLSFP_WORKER="unix://$SOCK_PATH"
...
case "$TRANSPORT" in
  subprocess)
    ...
    # VGI_TLSFP_WORKER (e.g. a bare command) if the caller set one.
    export VGI_TLSFP_WORKER="${VGI_TLSFP_WORKER:-$WORKER_BIN}"
    ;;
  http)
    # Honor a pre-launched HTTP worker (e.g. a running container in the docker
    # image_test): if VGI_TLSFP_WORKER already points at an http(s) URL, use it
    # and skip spawning a local binary. The awk preprocessor still injects
    # httpfs because TRANSPORT=http.
    if [[ "${VGI_TLSFP_WORKER:-}" =~ ^https?:// ]]; then
      echo "Using pre-launched HTTP worker at $VGI_TLSFP_WORKER"
    else
...
: "${VGI_TLSFP_WORKER:?worker LOCATION (stdio command, http:// URL, or unix:// socket)}"
...
echo "Running suite (transport: $TRANSPORT, worker: $VGI_TLSFP_WORKER, pattern: $TEST_PATTERN) ..."
...
if grep -qiE "All tests were skipped|total skipped [1-9]" "$REPORT"; then
  ...
  echo "       is NOT a pass. Transport=$TRANSPORT worker=$VGI_TLSFP_WORKER." >&2
  exit 1
fi
