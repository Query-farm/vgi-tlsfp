# CI: the vgi-tlsfp worker integration suite
...
(`test/sql/*.test`) against the vgi-tlsfp VGI worker through the **real DuckDB
...
| `TRANSPORT`  | `VGI_TLSFP_WORKER` (the ATTACH LOCATION) | how the worker is launched |
|--------------|------------------------------------------|----------------------------|
| `subprocess` | `…/target/release/tlsfp-worker`          | DuckDB spawns the stdio binary (default) |
| `http`       | `http://127.0.0.1:<port>`                | `tlsfp-worker --http` (auto port; prints `PORT:<n>` on stdout, which the script polls for) |
| `unix`       | `unix:///tmp/units.<pid>.sock`           | `tlsfp-worker --unix <sock>` (prints `UNIX:<sock>` on stdout + creates the socket; the script waits for both) |
...
1. **Build the worker** — `cargo build --release --bin tlsfp-worker`. The
   compiled `target/release/tlsfp-worker` is a self-contained stdio worker the
   extension spawns (the `.test` files `ATTACH` it via `${VGI_TLSFP_WORKER}`).
...
3. **Preprocess** — the standalone runner links none of the extensions the
   ...
   through untouched. (The vgi-tlsfp `.test` files already `LOAD vgi;`
   explicitly and use `require-env VGI_TLSFP_WORKER`, so the `require`-rewrite is
...
4. **Run** — [`run-integration.sh`](run-integration.sh) brings up the worker for
   the selected `TRANSPORT` and sets `VGI_TLSFP_WORKER` accordingly, stages the
...
```bash
cargo build --release --bin tlsfp-worker
...
# HTTP leg: launches `tlsfp-worker --http`, reads PORT:<n>, attaches http://…
HAYBARN_UNITTEST=/path/to/haybarn-unittest WORKER_BIN="$PWD/target/release/tlsfp-worker" \
  TRANSPORT=http ci/run-integration.sh

# Unix leg: launches `tlsfp-worker --unix /tmp/units.<pid>.sock`, attaches unix://…
HAYBARN_UNITTEST=/path/to/haybarn-unittest WORKER_BIN="$PWD/target/release/tlsfp-worker" \
