...
#
# Dispatch the single vgi-tlsfp image into one of its transports:
...
#
# The worker is stateless (pure fingerprint compute), so there is no /data to
...
case "${1:-http}" in
  http)
    ...
    exec tlsfp-worker --http "$@"
    ;;
  tcp)
    shift 2>/dev/null || true
    exec tlsfp-worker --tcp "0.0.0.0:${PORT_TCP:-8001}" "$@"
    ;;
  stdio)
    shift 2>/dev/null || true
    exec tlsfp-worker "$@"
