#!/usr/bin/env bash
# Build the browser (`worker:`) build of the tlsfp VGI worker: tlsfp-wasm
...
#     recompiles compiler_builtins with atomics (add rust-src to nightly with
...
LIB="target/$TARGET/release/libtlsfp_wasm.a"
...
# threads convoy on it; mimalloc is thread-caching.
#
# STACK_SIZE / DEFAULT_PTHREAD_STACK_SIZE: emscripten defaults to a 64 KiB stack,
# which the Arrow encoders + the SDK can overflow (surfacing as a bare "memory
# access out of bounds" with no stack trace). INITIAL_MEMORY must cover main
