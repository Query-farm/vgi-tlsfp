...
//! the `tlsfp-wasm` crate (the browser build, which serves the same `Worker`
...
/// The catalog name DuckDB sees in `ATTACH 'tlsfp' (TYPE vgi, …)`. Defaults to
/// `tlsfp`, but honors an explicit override so a test harness can rename it.
